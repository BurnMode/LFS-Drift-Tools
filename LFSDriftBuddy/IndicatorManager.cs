...
{/// <summary>/// Zarządza kierunkowskazami (lewy, prawy, awaryjny) z obsługą klawiszy,
 ...
 /// </summary>
    public class IndicatorManager : IDisposable
    {
        ...
        // ── Realny stan kontrolek kierunkowskazów z OutGauge (ShowLights) ─────
        // To jest PRAWDA z deski rozdzielczej LFS — może różnić się od CurrentState
        // (naszej intencji przełącznika), np. jeśli LFS sam anuluje kierunkowskaz.
        // Dźwięki (click/cancel) powinny być zsynchronizowane z TĄ wartością, nie z CurrentState.
        public bool LeftLampOn { get; private set; } = false;
        public bool RightLampOn { get; private set; } = false;

        // Niektóre auta w LFS nie wystawiają osobnych bitów DL_SIGNAL_L/DL_SIGNAL_R, tylko
        // jeden wspólny DL_SIGNAL_ANY — bez tego pola takie auta w ogóle nie miały wykrywanej
        // kontrolki (LeftLampOn/RightLampOn zawsze false), mimo że kierunkowskaz realnie migał.
        public bool AnySignalLampOn { get; private set; } = false;

        public bool AnyLampOn => LeftLampOn || RightLampOn || AnySignalLampOn;

        /// <summary>
        /// Zdarzenie odpalane wyłącznie przy realnej zmianie stanu kontrolki (zapalenie/zgaśnięcie)
        /// na podstawie danych z OutGauge — argument true = kontrolka właśnie się zapaliła,
        /// false = kontrolka właśnie zgasła (po wcześniejszym zapaleniu). To jest jedyne właściwe
        /// źródło synchronizacji dźwięków indicator_click_on.wav / indicator_click_off.wav.
        /// </summary>
        public event Action<bool>? LampStateChanged;

        /// <summary>
        /// Wywoływane przy każdym odebranym pakiecie OutGauge (patrz RevLimiter.DataReceived /
        /// OutGaugeData.LeftSignalOn / RightSignalOn / AnySignalOn) — aktualizuje realny stan
        /// kontrolek kierunkowskazów i odpala LampStateChanged TYLKO przy faktycznej zmianie
        /// (zapaleniu lub zgaśnięciu), żeby nie retriggerować dźwięku co pakiet UDP.
        /// </summary>
